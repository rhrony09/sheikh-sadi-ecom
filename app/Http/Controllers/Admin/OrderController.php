...
class OrderController extends Controller {
    ...
    public function get_stock_details(Request $request) {
        ...
        return view('backEnd.reports.stock-modal-content', ['stock' => $product]);
    }

    public function adjust_stock(Request $request, $id) {
        $request->validate([
        ...
        if ($request->input('adjustment_type') == 'in') {
            $stock->stock += $request->input('new_quantity');
        } else {
            if ($stock->stock >= $request->input('new_quantity')) {
                $stock->stock -= $request->input('new_quantity');
            } else {
                return response()->json(['success' => false, 'errors' => ['new_quantity' => ['Not enough stock available.']]]);
            }
        }

        $stock->save();

        Toastr::success('Success', 'Stock adjusted successfully!');
